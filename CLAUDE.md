...
Если изменения в omnimap-sync требуют изменений в других сервисах:

1. **НЕ редактируй** файлы в `omnimap-back`, `omnimap-front` или `llm-gateway`
2. **Создай файл задач** `FRONTEND_TASKS.md` или `BACKEND_TASKS.md` в корне этого репозитория:
   ```markdown
   # Задачи для других сервисов

   ## omnimap-front
   - [ ] Обновить WebSocket клиент для нового формата сообщений
   - [ ] Добавить обработку нового типа события

   ## omnimap-back
   - [ ] Изменить формат отправляемых RabbitMQ сообщений

   ## llm-gateway
   - [ ] Добавить интеграцию с sync для стриминга
   ```
...
Проверь файл `SYNC_TASKS.md` (если существует) — там могут быть задачи от фронтенда или других сервисов.
